...
export function LoreMapPanel() {
  return (
    <div className="premium-panel p-4 sm:p-5">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_28%_20%,rgba(204,100,55,0.14),transparent_30%),radial-gradient(circle_at_72%_72%,rgba(204,100,55,0.10),transparent_32%),radial-gradient(circle_at_50%_50%,rgba(245,242,237,0.08),transparent_38%)]" />
      ...
      <div className="relative grid gap-4 lg:grid-cols-[1.15fr_0.85fr] lg:items-stretch">
        <div className="sub-card relative min-h-[280px] overflow-hidden p-3 sm:min-h-[330px]">
          ...
          <svg viewBox="0 0 440 320" className="relative h-full min-h-[260px] w-full text-cyan-100" fill="none" aria-hidden="true">
            ...
            <path d="M82 254C116 230 144 236 169 264" stroke="#cc6437" strokeWidth="1.4" strokeLinecap="round" opacity="0.42" />
            <path d="M182 151C212 168 236 168 265 146" stroke="#c4beb7" strokeWidth="1.5" strokeLinecap="round" opacity="0.40" />
            <path d="M274 150C304 138 324 120 338 92" stroke="#e4a080" strokeWidth="1.2" strokeLinecap="round" opacity="0.34" />
            <path d="M116 88h78M130 105h44M278 232h84M294 248h42" stroke="#e4a080" strokeWidth="1" strokeLinecap="round" opacity="0.22" />
            <path d="M58 72 92 48 126 72 92 98 58 72Z" stroke="#cc6437" strokeWidth="1" opacity="0.22" />
            <path d="M314 58 350 30 386 58 350 86 314 58Z" stroke="#e4a080" strokeWidth="1" opacity="0.24" />
            <circle cx="44" cy="248" r="8" fill="#e4a080" opacity="0.78" />
            <circle cx="174" cy="154" r="7" fill="#cc6437" opacity="0.72" />
            <circle cx="294" cy="148" r="7" fill="#c4beb7" opacity="0.72" />
            <circle cx="398" cy="82" r="8" fill="#6ee7b7" opacity="0.66" />
          </svg>

          <div className="absolute left-[9%] top-[72%] h-9 w-9 rounded-xl border border-cyan-200/15 bg-[#080908]/80 p-2 backdrop-blur-md">
            <GameGlyph name="fog" variant="plain" className="h-full w-full text-cyan-100" />
          </div>
          <div className="absolute left-[38%] top-[43%] h-9 w-9 rounded-xl border border-amber-200/15 bg-[#080908]/80 p-2 backdrop-blur-md">
            <GameGlyph name="ruin" variant="plain" className="h-full w-full text-amber-100" />
          </div>
          <div className="absolute left-[65%] top-[42%] h-9 w-9 rounded-xl border border-purple-200/15 bg-[#080908]/80 p-2 backdrop-blur-md">
            <GameGlyph name="dash" variant="plain" className="h-full w-full text-purple-100" />
          </div>
          <div className="absolute right-[8%] top-[20%] h-9 w-9 rounded-xl border border-emerald-200/15 bg-[#080908]/80 p-2 backdrop-blur-md">
        ...
        <div className="grid gap-3">
          ...
          {interestPoints.map((point) => (
            <div key={point.label} className="sub-card flex items-start gap-3 px-4 py-3">
