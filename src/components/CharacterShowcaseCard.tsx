...
export type CharacterVisualKind = "rubens" | "lucarelli" | "enemy" | "future" | "planned";
...
function AbstractSilhouette({ kind, icon }: { kind: CharacterVisualKind; icon: GameGlyphName }) {
...
export function CharacterShowcaseCard({ character }: { character: CharacterShowcase }) {
  const style = visualStyles[character.visualKind];
  ...
  return (
    <GlowCard contentClassName="flex h-full flex-col">
      ...
      <div className="mt-4">
        <p className="text-xs uppercase tracking-[0.16em] text-cyan-200/75">{character.functionLabel}</p>
        <h3 className="mt-1 text-2xl font-bold text-white">{character.name}</h3>
        <p className="mt-3 text-sm leading-6 text-slate-300">{character.description}</p>
      </div>

      <div className="mt-5 rounded-xl border border-cyan-200/10 bg-black/20 px-4 py-3">
        <p className="text-xs uppercase tracking-[0.14em] text-slate-400">Papel no beta</p>
        <p className="mt-2 text-sm leading-6 text-slate-200">{character.betaRole}</p>
      </div>

      <div className="mt-4">
        <p className="text-xs uppercase tracking-[0.14em] text-slate-400">Leitura rápida</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {character.abilities.map((ability) => (
            <span key={ability} className="rounded-lg border border-white/10 bg-white/[0.035] px-3 py-1.5 text-xs text-slate-300">
              {ability}
            </span>
          ))}
        </div>
