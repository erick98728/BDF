...
export default async function DevlogPostPage({ params }: Props) {
  ...
  return (
    <AnimatedPageWrapper>
      ...
      <SectionContainer>
        <GlowCard contentClassName="p-5 sm:p-7">
          ...
          <p className="mt-5 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">{post.summary}</p>
        </GlowCard>
      </SectionContainer>
      ...
      <SectionContainer withDivider>
        <div className="grid gap-5">
          {post.content.map((section) => (
            <GlowCard key={section.heading} contentClassName="p-5 sm:p-6">
      ...
      <SectionContainer withDivider>
        <div className="grid gap-4 lg:grid-cols-2">
          <DevlogListCard title="Progresso registrado" items={post.progress} tone="cyan" />
          <DevlogListCard title="Próximos passos" items={post.nextSteps} tone="gold" />
        </div>
      </SectionContainer>
    </AnimatedPageWrapper>
  );
...
function DevlogListCard({ title, items, tone }: { title: string; items: string[]; tone: "cyan" | "gold" }) {
  const markerClass = tone === "cyan" ? "bg-cyan-200" : "bg-amber-200";

  return (
    <GlowCard contentClassName="p-5 sm:p-6">
      <SectionTitle title={title} subtitle="Registro honesto do estado atual, sem promessa de versão final." />
