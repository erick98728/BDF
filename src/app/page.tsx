  ...
  return (
    <div className="region-map" aria-hidden="true">
      ...
      <svg viewBox="0 0 560 380" className="region-map__drawing" fill="none">
        <path d="M35 304C94 238 153 286 202 211C250 139 312 151 352 202C391 252 440 204 521 103" className="region-map__route" />
        <path d="M202 211C167 170 136 145 82 138" className="region-map__route region-map__route--secondary" />
        <path d="M352 202C388 170 422 154 467 157" className="region-map__route region-map__route--secondary" />
        <path d="M96 324C131 296 162 304 188 334M381 282C424 254 462 261 501 292" className="region-map__branch" />
        <path d="M97 72h114M121 91h66M367 327h116M393 346h67" className="region-map__record" />
        <path d="M48 119 83 93 118 119 83 146 48 119ZM451 79 486 52 521 79 486 106 451 79Z" className="region-map__ruin" />
        <circle cx="35" cy="304" r="9" className="region-map__node" />
        <circle cx="202" cy="211" r="7" className="region-map__node region-map__node--ember" />
        <circle cx="352" cy="202" r="7" className="region-map__node" />
        <circle cx="521" cy="103" r="10" className="region-map__node region-map__node--ember" />
      </svg>
    ...
    <AnimatedPageWrapper>
        ...
        <div className="editorial-overview">
          <div className="editorial-overview__story">
            <div className="editorial-lead">
              <GameGlyph name="fog" variant="plain" className="editorial-lead__glyph" />
              <div>
            ...
            <div className="editorial-pillars">
              {homePillars.map((pillar) => (
                <article key={pillar.title} className="editorial-pillar">
                  <span className="editorial-pillar__line" aria-hidden="true" />
                  <div>
                    <p className="editorial-label">{pillar.eyebrow}</p>
                    <h3>{pillar.title}</h3>
                    <p>{pillar.description}</p>
                  </div>
                  <GameGlyph name={pillar.icon} variant="plain" className="editorial-pillar__glyph" />
                </article>
              ))}
            </div>
          </div>

          <aside className="current-state">
      ...
      <SectionContainer withDivider>
        ...
        <ol className="system-index">
          {homeFeatures.map((feature, index) => (
            <li key={feature.title} className="system-index__item">
              <span className="system-index__number">{String(index + 1).padStart(2, "0")}</span>
              <GameGlyph name={feature.icon} variant="plain" className="system-index__glyph" />
              <div>
                <h3>{feature.title}</h3>
      ...
      <SectionContainer withDivider>
        ...
        <div className="region-spread">
          <AbstractMapPanel />
          <article className="region-record">
            <GameGlyph name="ruin" variant="plain" className="region-record__glyph" />
      ...
      <SectionContainer withDivider>
        ...
        <div className="editorial-roster">
          {characters.map((c) => (
            <article key={c.name} className="editorial-roster__entry">
              <GameGlyph name={c.icon} variant="plain" className="editorial-roster__glyph" />
      ...
      <SectionContainer withDivider>
        <div className="editorial-callout">
          <GameGlyph name="feedback" variant="plain" className="editorial-callout__glyph" />
          <div>
      ...
      <SectionContainer withDivider>
        ...
        <div className="editorial-log">
          {devlogs.map((d, index) => (
            <article key={d.title} className="editorial-log__entry">
              <span className="editorial-log__index">{String(index + 1).padStart(2, "0")}</span>
              <div>
                <p className="editorial-label">{d.date}</p>
                <h3>{d.title}</h3>
                <p>{d.excerpt}</p>
              </div>
              <GameGlyph name={d.icon} variant="plain" className="editorial-log__glyph" />
            </article>
          ))}
        </div>
      </SectionContainer>
